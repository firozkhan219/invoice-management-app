...
import { addInvoiceItem } from "@/lib/invoices/invoice-service";
...
export async function POST(request: Request) {
  ...
  const parsed = invoiceItemSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  await addInvoiceItem(context, parsed.data);
