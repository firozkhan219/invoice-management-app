...
import { updateInvoiceItem } from "@/lib/invoices/invoice-service";
import { getTenantContextForUser } from "@/lib/organisations/membership";
import { invoiceItemUpdateSchema } from "@/lib/validation/invoices";
...
export async function POST(request: Request) {
  ...
  const parsed = invoiceItemUpdateSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  try {
    await updateInvoiceItem(context, parsed.data);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not update invoice line.";
