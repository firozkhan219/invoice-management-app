...
import { cancelInvoiceSchema } from "@/lib/validation/invoices";
...
export async function POST(request: Request) {
  ...
  const parsed = cancelInvoiceSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  let destination: string;

  try {
    const invoice = await cancelInvoice(context, parsed.data);
