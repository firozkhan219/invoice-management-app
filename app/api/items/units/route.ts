...
import { createUnit } from "@/lib/items/item-service";
import { getTenantContextForUser } from "@/lib/organisations/membership";
import { unitSchema } from "@/lib/validation/items";
...
export async function POST(request: Request) {
  ...
  const parsed = unitSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  await createUnit(context, parsed.data);
  redirect("/items");
}
