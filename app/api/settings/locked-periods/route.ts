...
import { createLockedPeriod } from "@/lib/settings/settings-service";
import { lockedPeriodSchema } from "@/lib/validation/settings";
...
export async function POST(request: Request) {
  ...
  const parsed = lockedPeriodSchema.safeParse(Object.fromEntries(await request.formData()));
  if (!parsed.success) return Response.json({ ok: false, errors: parsed.error.flatten().fieldErrors }, { status: 400 });

  await createLockedPeriod(context, parsed.data);
  redirect("/settings");
}
